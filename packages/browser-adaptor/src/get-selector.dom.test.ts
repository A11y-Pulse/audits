...
/**
 * These tests exercise getSelector against a real DOM, where the property that matters is
 * uniqueness: the selector it returns must resolve to exactly the element it was built from. The
 * pure-logic tests in get-selector.test.ts cover the serialised, sibling-unaware behaviour with
 * fake nodes.
 */
...
describe("getSelector uniqueness", () => {
	it("disambiguates identical-class siblings with no id ancestor", () => {
		// Mirrors the tyoelake.fi footer navigation: sibling <li>s with identical classes, each
		// wrapping an <a> with identical classes, and no id anywhere in the ancestry to anchor on.
	...
	it("does not anchor on a non-unique id", () => {
		// Mirrors tyoelake.fi heading links: the page repeats the same id on distinct headings, so
		// "#dup>a" resolves to more than one element.
		document.body.innerHTML = `
	...
	it("still anchors on a genuinely unique id", () => {
		// The decoy section makes every level below the id ambiguous, so the walk must genuinely reach
		// and anchor on #panel to disambiguate.
		document.body.innerHTML = `
...
describe("getSelector minimal segments", () => {
	it("uses a single discriminating class rather than every class", () => {
		// Mirrors WordPress post cards: a wall of shared utility classes plus one class that is unique
		// to the element.
		document.body.innerHTML = `
	...
	it("stops walking as soon as the selector is unique", () => {
		// The target's class is unique on the page, so no ancestor segments are needed at all. Matching
		// how axe-core selectors stay short.
		document.body.innerHTML = `
	...
	it("discriminates structurally-identical content by href", () => {
		// Three sections with identical classes and positions inside separate wrappers: nth-of-type
		// pins position within each parent, so it can't tell the copies apart. But their hrefs differ,
		// as on real pages.
	...
	it("keeps room for ancestor context when an href is not unique", () => {
		// The same long href appears twice (e.g. a card link and a nav link), so it can't pin either
		// element on its own. Disambiguating ancestors must still fit within the budget alongside the
		// ~100-character href.
	...
	it("does not build href features from values with CSS escape characters", () => {
		// A backslash in an attribute value is a CSS string escape, so the selector can parse to a
		// different string than the attribute holds. Matching nothing, or worse, a different element
		// entirely.
	...
	it("does not build href features from volatile query-string URLs", () => {
		// Query strings change between runs (pagination, session ids), which would destabilise the
		// selector; positional pins handle these instead.
		document.body.innerHTML = `
	...
	it("rejects a feature the element itself does not match", () => {
		// Simulates an engine/escaping mismatch: if the element doesn't match a candidate feature, that
		// feature can select the wrong element and must not be used.
		document.body.innerHTML = `
		...
		(link as { matches: (selectors: string) => boolean }).matches = (selector: string) =>
...
describe("getSelector length budget", () => {
	it("caps the selector at 128 characters and still matches the element", () => {
		// Deeply nested markup where every level shares the same classes, so uniqueness is never
		// reached and the walk is bounded by the budget; the capped selector may not be unique, but it
		// must still match the element.
	...
	it("degrades a leaf segment that alone exceeds the budget to tag and position", () => {
		// A single class longer than the whole budget: the selector must stay non-empty, within budget,
		// and unique via the position pin.
