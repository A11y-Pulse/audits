...
/** A SkipLinkAuditAdaptor backed by a Puppeteer Page. */
export class PuppeteerAdaptor implements SkipLinkAuditAdaptor {
	...
	async ensureFocusReporting(): Promise<void> {
		...
		try {
			...
			console.warn("Could not enable focus emulation for the skip link audit", {
				error,
			});
		}
	}
