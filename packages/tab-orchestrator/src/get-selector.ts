 ...
 * as soon as the joined selector matches exactly one element. Selectors stay short because of that
 * early stop, not because of truncation; the length budget is only a backstop.
 */
export function getSelector(root: Node | null): string {
	...
	// A querySelectorAll-capable root (Document or ShadowRoot) for the element, used to confirm
	// uniqueness and class rarity. Absent on the plain objects used in the serialisation unit tests,
	// where we trust ids and walk to the top instead of stopping early.
	...
	// A candidate feature is only usable if it actually selects this element; an escaping or engine
	// mismatch (e.g. CSS string escapes in an attribute value) could otherwise produce a selector
	// that matches nothing. Or a different element entirely.
	...
	// A discriminating feature for one level of the selector: its CSS text plus a predicate telling
	// whether a sibling would match the same feature (used to decide if an :nth-of-type pin is still
	// needed).
	type Feature = {
		feature: string;
	...
	// The single feature that narrows the element down the most: the class or href attribute with the
	// fewest matches in the root. Hrefs break ties between structurally-identical content blocks,
	// where classes and :nth-of-type (which only pins position within one parent) cannot. Without a
	// queryable root there is no rarity information, so the first class stands in.
	const rarestFeature = (element: Element, queryRoot: ParentNode | null): Feature | null => {
		...
		// Skip hrefs with characters that change meaning inside a CSS string (quotes, backslashes), and
		// query strings, which are too volatile across runs to pin a selector to. Fragments (#main)
		// stay eligible. Skip links make stable discriminators.
	...
	// The element's 1-based position among same-tag siblings, and whether the chosen feature fails to
	// discriminate it from them (so the segment needs an :nth-of-type pin). Siblings are absent on
	// the serialisation test nodes. There we skip the pin.
	...
	const segmentFor = (
		...
		// Only treat an id as an absolute anchor when it actually resolves to a single element: pages
		// with duplicate ids would otherwise yield a selector (e.g. "#dup>a") that matches several
		// elements.
		...
		// When same-tag siblings share the chosen feature, pin the position so the segment selects
		// exactly this element rather than all of them.
	...
	// A minimal segment for when a level's full segment would blow the budget (e.g. an enormous class
	// name or a long href): just the tag, pinned by position when same-tag siblings would make the
	// bare tag ambiguous.
	...
	try {
		// Walk only element nodes. Stopping at the first non-element parent (the document, or a
		// ShadowRoot when the target is inside an open shadow root) keeps the selector within its tree
		// and avoids reading `classList` off a node that has none.
		while (node && node.nodeType === 1) {
			const element = node as Element;
			let { segment, anchored } = segmentFor(element, queryRoot);

			// Keep within the budget (the +1 accounts for the ">" that would join this segment to the
			// rest). A level whose full segment doesn't fit is degraded to tag + position rather than
			// discarded, so ancestor context is never lost to one long feature.
			...
			// Stop as soon as the selector already identifies a single element. This, not truncation, is
			// what keeps selectors short.
