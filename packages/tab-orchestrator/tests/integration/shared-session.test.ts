...
describeEachEngine<BrowserAdaptor>(
	"shared session across three audits (integration)",
	{ adaptor: adaptors, screenshotClipScale: 2 },
	(engine) => {
		it("tabs once and stops screenshotting after appearance disconnects", async () => {
			const { adaptor: inner, goto, close, keyPresses } = await engine.newPage();
			let clips = 0;
			const adaptor: BrowserAdaptor = {
				...
				pressEnter: inner.pressEnter.bind(inner),
			...
			await goto(`${server.url}/mixed-focus.html`);
			...
			const tabs = keyPresses.filter((key) => key === "Tab").length;
